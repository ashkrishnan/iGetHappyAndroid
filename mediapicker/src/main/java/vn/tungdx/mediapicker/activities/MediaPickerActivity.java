 ...
 * {@link MediaPickerActivity#open(AppCompatActivity, int, MediaOptions)} or
 * {@link MediaPickerActivity#open(AppCompatActivity, int)}</li>
 ...
 * {@link AppCompatActivity#onActivityResult(int, int, Intent)} of activity or fragment
 ...
 * to {@link #open(AppCompatActivity, int, MediaOptions)} </i></li>
 * </ul>
 */
public class MediaPickerActivity extends AppCompatActivity implements
    ...
    private static final String EXTRA_MEDIA_SELECTED_PREVIOUS = "extra_media_selected_previous";
    ...
    /**
     * Start {@link MediaPickerActivity} in {@link AppCompatActivity} to pick photo or
    ...
    /**
     * Start {@link MediaPickerActivity} in {@link AppCompatActivity} with default media
     ...
     *
     * @param intent In {@link AppCompatActivity#onActivityResult(int, int, Intent)} method of
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mMediaOptions != null && mMediaOptions.getMediaListSelected().size() > 0 && mMediaOptions.getMediaListSelected().get(0).getType() == 1) {
            tvCount.setVisibility(View.VISIBLE);
            tvCount.setText(mMediaOptions.getMediaListSelected().size() + " selected");
        }
    }
    ...
    public void onHasSelected(List<MediaItem> mediaSelectedList) {
        if (mediaSelectedList.get(0).getType() == 1)
            tvCount.setVisibility(View.VISIBLE);
        tvCount.setText(mediaSelectedList.size() + " selected");
        showDone();
    }
    ...
    public void syncActionbar() {
        ...
        if (fragment instanceof PhotoCropFragment) {
            ...
            if (pickerFragment.hasMediaSelected()) {
                if (pickerFragment.getMediaSelectedList().get(0).getType() == 1)
                    tvCount.setVisibility(View.VISIBLE);
                showDone();
            } else {
                tvCount.setVisibility(View.GONE);
    ...
    private void returnVideo(Uri videoUri) {
        ...
        switch (code) {
            ...
            case -2:
                showVideoInvalid("Video Size should  not be more than 100mb");
                break;
            // smaller than min
            case -1:
                // in seconds

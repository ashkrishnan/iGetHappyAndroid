...
class EditPostRepository {
    ...
    fun updatePost(
        post_id: String,
        updatePostInput: HashMap<String, RequestBody>,
        allData: MutableLiveData<AddPostResponse.Data>,
        mIsUpdating: MutableLiveData<Boolean>,
        file: MultipartBody.Part
    ) {
        val call = GetRestAdapter.getRestAdapter(false).updatePost(post_id, updatePostInput, file)
    ...
    fun updateTextPost(
        ...
        val call = GetRestAdapter.getRestAdapter(false).updateTestPost(post_id, model)
        call.enqueue(object : Callback<AddPostResponse> {
            override fun onResponse(
                call: Call<AddPostResponse>,
                response: Response<AddPostResponse>?
